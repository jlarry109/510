...
public:
  Board(int w, int h, int numMines):  board(NULL), width{w}, height{h}, totalMines{numMines} {
      /* write code*/
  ...
  int countMines(int x, int y) {
    ...
    for (int dy = -1; dy <= 1 ; dy++) {
        for (int dx = -1; dx <=1 ; dx++) {
            int nx = x + dx;
            int ny = y + dy;
            numMines = (in_bounds(nx,ny) && board[ny][nx].is_mine())? numMines + 1: numMines;
