...
std::pair<std::vector<double>, std::vector<std::vector<double>>> readFiles(const std::string &universeFileName, const std::string &correlationFileName) {
    ...
    while (universeStream.good() && std::getline(universeStream, line)) {
        ...
        exitWithFailure(tokens.size() != 3, "Invalid formatting in universe file: missing entries");
    ...
    for (int i = 0; i < (int)sd.size(); ++i) {
        ...
        for (int j = 0; j < (int)sd.size(); ++j) {
            if (!getline(ssline, token, ',')) {
                std::cerr << "Incorrect file format specified.." << std::endl;
                exit(EXIT_FAILURE);
            }
            try {
        ...
        exitWithFailure(i > 0 && cov[i].size() != cov[i - 1].size(), "Invalid entries in correlation file: missing entries");
        if (getline(ssline, token, ',')) {
            std::cerr << "Incorrect file format specified..." << std::endl;
            exit(EXIT_FAILURE);
        }
...
void solveAndPrint(const std::vector<double> &averageReturn, const std::vector<std::vector<double>> &cov, const bool r) {
